...
python scripts/make_portrait.py source/face_crop.jpg --invert --contrast 0.9 --gamma 1.4 --detail 0.5 --brow-darken 2 --cols 140 --oval 0.95
```
Portrait flags: `--cols` detail, `--invert` bright=dense, `--gamma/--contrast` tone, `--detail` local
contrast (eyes/lips), `--brow-darken` deepens existing eyebrow strokes, `--oval` blank outside a
head-shaped ellipse, `--bg-cut` blank the background colour.
